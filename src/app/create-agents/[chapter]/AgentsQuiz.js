...
export default function AgentsQuiz({ questions, chapterId, chapterTitle }) {
  ...
  const handleSubmit = () => {
    ...
    setSubmitted(true);
    const saved = JSON.parse(localStorage.getItem("agents_quiz_scores") || "{}");
    saved[`chapter_${chapterId}`] = score;
    localStorage.setItem("agents_quiz_scores", JSON.stringify(saved));
  };
  ...
  return (
    <div className="bg-white border-2 border-orange-200 rounded-2xl p-6 md:p-8 mb-10">
          ...
          return (
            <div key={qIdx} className={`rounded-xl p-4 border ${
              ...
              <p className="font-semibold text-slate-800 mb-3">
                <span className="text-orange-500 mr-2">{qIdx + 1}.</span>{q.q}
              </p>
              <div className="space-y-2">
                {q.options.map((opt, optIdx) => {
                  ...
                  if (!submitted) {
                    optClass += isSelected ? "border-orange-500 bg-orange-50 text-orange-700" : "border-slate-200 hover:border-orange-300 hover:bg-slate-50";
                  } else {
                  ...
                  return (
                    <div key={optIdx} className={optClass} onClick={() => handleSelect(qIdx, optIdx)}>
                      <span className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                        !submitted ? isSelected ? "border-orange-500 bg-orange-500 text-white" : "border-slate-300"
      ...
      <div className="mt-6">
        {!submitted ? (
          <button onClick={handleSubmit} className="w-full bg-orange-500 text-white font-bold py-3 rounded-xl hover:bg-orange-600 transition-colors">
            Submit Answers
          </button>
        ) : (
